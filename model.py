...
def cnn(input_dim, vocab_size,maxLen, embedding_dim, hidden_dim, ouput_dim, keep_prob=0.2):
    model = Sequential()
    model.add(Embedding(input_dim=vocab_size, output_dim=embedding_dim, input_length=maxLen))
    model.add(BatchNormalization())
    model.add(Dropout(rate=keep_prob))
    model.add(Conv1D(filters=ouput_dim, kernel_size=embedding_dim, activation='relu', kernel_regularizer= regularizers.l1(1e-1)))
    # model.add(Flatten())
    model.add(BatchNormalization())
    model.add(Dropout(rate=keep_prob))
    model.add(MaxPool1D())
    model.add(Flatten())
    model.add(Dense(ouput_dim,activation='relu'))
    model.add(Dense(ouput_dim, activation='sigmoid', activity_regularizer=regularizers.l1(1e-4)))
    return model
